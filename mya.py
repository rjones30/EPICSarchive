...
def lookup(varname, deployment="ops"):
   ...
   if deployment == "ops":
      ...
      return {}
   if not host in db:
      print("connecting to host", host, "...", end='')
      connect(host)
      print("done!")
   cur = db[host].cursor()
...
def fetch(descr, t0, dt=0, cond=None):
   ...
   if key in epics_cache:
      ...
      elif t0 < epics_cache[key][1][-1] and t1 > epics_cache[key][1][-1]:
         ddt = (t1 - epics_cache[key][1][-1]) / epics_second
         values, times = fetch(descr, epics_cache[key][1][-1], ddt)
         appended = 0
         for row in zip(values, times):
            if row[1] > epics_cache[key][1][-1]:
               epics_cache[key][0].append(row[0])
               epics_cache[key][1].append(row[1])
               appended += 1
         if appended:
            return fetch(descr, t0, dt, cond)
         else:
            return fetch(descr, t0, dt - ddt - 1, cond)
   ...
   if not host in db:
      del epics_cache['mya descriptors'][descr['name']]
      lookup(descr['name'], descr['deployment'])
   if not host in db:
      connect(host)
   cur = db[host].cursor()
...
def filter_cond(values, times, cond):
   ...
   moves the time value forward, but never inserts new elements. A prefetch
   is performed on all epics variables in the cond query expression before
   it is applied to the individual times, otherwise individual db select
   requests will bleed out the mya archive db with a thousand paper cuts.
   """
   if not cond or len(times) == 0:
      return values, times
   for e in re.finditer(r"[A-Za-z][.A-Za-z:_0-9]*", cond):
      fetch(e.group(0), times[0], (times[-1] - times[0]) // epics_second)
   fvalues = []
   ftimes = []
   i = 0
   for r in find_ranges(cond, times[0], times[-1]):
...
def search_ranges(query, t0, t1):
   ...
   if not pquery:
      ...
      for i in range(1, len(pquery)):
         ...
         if level == 0:
            subj = pquery[1:i]
            subj_ranges = [r for r in search_ranges(subj, t0, t1)]
            break
      if level != 0:
...
def connect(host):
   ...
   dbuser = "myapi"
   dbpasswd = "MYA"
   print("debug: connect to port {0} on {1}".format(port[host],dbproxy))
...
def cachein(cachefile="mya.cache"):
   """
   Reads a pickled copy of epics_cache from a previous run, saves having
   to go out and fetch long records from the archive for repeated runs
   over the same variables. This file should previously have been saved
   by cacheout(filename).
   """
   global epics_cache
   fcache = open(cachefile, 'rb')
   epics_cache = pickle.load(fcache)

def cacheout(cachefile="mya.cache"):
   """
   Writes a pickled copy of epics_cache to a file, to save having to
   go out and fetch long records from the archive again for repeated runs
   over the same variables. Use cachein to recover the saved records.
   """
   fcache = open(cachefile, 'wb')
   pickle.dump(epics_cache, fcache)
